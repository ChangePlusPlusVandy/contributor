...
async def vendor_login(data: VendorLoginRequest):
    logger.info(f"Vendor login attempt for vendor_id={data.vendor_id}")
    vendors = get_vendor_users_collection()
    vendor = await vendors.find_one({"vendor_id": data.vendor_id}, {"_id": 0})

    if not vendor:
        logger.warning(f"Failed vendor login: no vendor with vendor_id={data.vendor_id}")
    ...
    try:
        ...
        logger.warning(f"Failed vendor login for vendor_id={data.vendor_id}: invalid credentials")
        raise HTTPException(status_code=401, detail="Invalid Vendor ID or password")

    logger.info(f"Successful vendor login for vendor_id={data.vendor_id}")
    return {
...
async def change_vendor_password(data: VendorChangePasswordRequest, current_user: dict = Depends(get_current_user)):
    logger.info(f"Changing password for vendor_id={current_user.get('vendor_id')}")
    ...
    logger.info(f"Successfully changed password for vendor_id={current_user.get('vendor_id')}")
    return {
...
async def clock_in_vendor(current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"Clock-in requested by vendor_id={vendor_id}")
    vendors = get_vendor_users_collection()
    ...
    if clocked_in_time:
        ...
        if hours_clocked_in >= 4:
            ...
            logger.info(f"Auto clocked out vendor_id={vendor_id} after {hours_clocked_in:.1f} hours")
            return {"message": "Auto clocked out after 4 hours", "auto_clocked_out": True}

    if not current_user.get("location"):
        logger.warning(f"Clock-in rejected for vendor_id={vendor_id}: no location set")
    ...
    logger.info(f"Successfully clocked in vendor_id={vendor_id}")
...
async def clock_out_vendor(current_user: dict = Depends(get_current_user)):
    vendor_id = current_user.get("vendor_id")
    logger.info(f"Clock-out requested by vendor_id={vendor_id}")
    vendors = get_vendor_users_collection()
    await vendors.update_one(
        {"supabase_id": current_user.get("supabase_id")},
        {"$set": {"is_clocked_in": False}, "$unset": {"clocked_in_at": ""}}
    )
    logger.info(f"Successfully clocked out vendor_id={vendor_id}")
...
async def set_vendor_location(data: VendorLocationRequest, user: dict = Depends(get_current_user)):
    ...
    logger.info(f"Updated location for vendor_id={user.get('vendor_id')}")
...
async def debug_reset_vendor_password(vendor_id: str, data: VendorChangePasswordRequest):
    ...
    try:
        ...
        logger.error(f"Failed to reset password for vendor_id={vendor_id}: {e}", exc_info=True)
...
async def get_active_vendors():
    logger.info("Fetching active vendors...")
    vendors = get_vendor_users_collection()
    active = await vendors.find(
        {"is_clocked_in": True, "location": {"$ne": None}},
        {"_id": 0, "vendor_id": 1, "name": 1, "location": 1}
    ).to_list(length=None)
    logger.info(f"Successfully retrieved {len(active)} active vendors.")
