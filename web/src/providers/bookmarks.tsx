...
export const BookmarksProvider = ({ children }: { children: ReactNode }) => {
    const [bookmarkedOrgNames, setBookmarkedOrgNames] = useState<string[]>(
        () => parseStoredOrgNames(localStorage.getItem(STORAGE_KEY))
    );

    useEffect(() => {
        try {
            localStorage.setItem(STORAGE_KEY, JSON.stringify(bookmarkedOrgNames));
        } catch {
            // storage may be unavailable (private browsing quota) — bookmarks stay in memory
        }
    }, [bookmarkedOrgNames]);
